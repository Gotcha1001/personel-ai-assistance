...
// Define types for the color themes
type ColorTheme = string[];
type ColorThemes = {
  [key: string]: ColorTheme;
};

// Define types for shape definitions
type PolygonShape = {
  points: string;
  type: "polygon";
};
...
interface MandalaShape {
  ...
  shapePattern: "single" | "alternating";
...
const VaryingShapeMandalas: React.FC = () => {
  ...
  // Use useRef instead of useState for the counter to avoid re-renders and race conditions
  ...
  const generateMandalaShapes = (): void => {
    const newMandalas: MandalaShape[] = Array(1)
      ...
      .map(() => {
        ...
        // Create a truly unique ID by incrementing the counter reference
        const uniqueId = idCounterRef.current++;

        return {
          id: `varied-mandala-${uniqueId}`,
          centerX,
          centerY,
          size,
          duration,
                ...
                >
                  ...
                  {Array.from({ length: mandala.elements }).map(
                    (_, elementIndex) => {
                      ...
                      const strokeIndex =
                        (elementIndex + 1) % mandala.colors.length;

                      return (
                        <motion.div
                          key={`${mandala.id}-shape-${ringIndex}-${elementIndex}`}
                          style={{
                            position: "absolute",
                            left: 0,
                            top: 0,
                          ...
                          animate={{
                            ...
                            scale: 1,
                            opacity: 1,
                            rotate: mandala.hasRotatingElements
                              ? elementIndex % 2 === 0
                                ? 360
                                : -360
                              : 0,
                          ...
                          transition={{
                            duration: mandala.hasRotatingElements
                              ? mandala.duration * 0.8
                              : 1.5,
                            delay:
                              ringDelay +
